...
def logger_config(log_path):
    ...
    # File handler to log messages
    handler = logging.FileHandler(log_path, encoding='UTF-8')
    handler.setLevel(logging.INFO)
    formatter = logging.Formatter('%(message)s')
    handler.setFormatter(formatter)
    # Console handler to log messages
...
def save_checkpoint(state, save_path):
    '''
        ...
        it will be saved with a specific 'best_model' name.
    '''
...
def worker_init_fn(worker_id):
    """Initialize worker for DataLoader to ensure reproducibility."""
    random.seed(config.seed + worker_id)


##################################################################################
# =================================================================================
#          Main Loop: load model, data, optimizer, scheduler, and run training
# =================================================================================
##################################################################################
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    # Load datasets based on task name
    if config.task_name == 'MoNuSeg':
        # Load text descriptions for MoNuSeg dataset
        ...
        # Load text descriptions for Covid19 dataset
    ...
    if model_type == 'LViT':
        ...
        # Log transformer configuration details
        logger.info('transformer head num: {}'.format(config_vit.transformer.num_heads))
        logger.info('transformer layers num: {}'.format(config_vit.transformer.num_layers))
        logger.info('transformer expand ratio: {}'.format(config_vit.expand_ratio))
        # Create LViT model instance
        ...
        pretrained_UNet_model_path = "MoNuSeg/LViT/Test_session_/models/best_model-LViT.pth.tar"
        pretrained_UNet = torch.load(pretrained_UNet_model_path, map_location='cuda')
        pretrained_UNet = pretrained_UNet['state_dict']
        model2_dict = model.state_dict()
        # Filter pre-trained weights to match current model keys
    ...
    # Calculate model complexity (flops and params)
    ...
    if config.cosineLR is True:
        # CosineAnnealingWarmRestarts scheduler as proposed in SGDR paper
    ...
    for epoch in range(config.epochs):  # loop over the dataset multiple times
        ...
        # Train for one epoch (lr_scheduler is None for training)
        ...
        with torch.no_grad(): # Disable gradient computation for validation
            ...
            # Validate for one epoch (lr_scheduler is passed for potential update after validation)
        ...
        if val_dice > max_dice:
            if epoch + 1 > 5: # Only save best model after epoch 5
                logger.info(
                    '\t Saving best model, mean dice increased from: {:.4f} to {:.4f}'.format(max_dice, val_dice))
                max_dice = val_dice
                best_epoch = epoch + 1
                # Save checkpoint with best_model flag set to True
